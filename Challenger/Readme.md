...
WORKDIR /src
COPY ["CreateTokenLambda.csproj", "CreateTokenLambda/"]
RUN dotnet restore "CreateTokenLambda/CreateTokenLambda.csproj"

WORKDIR "/src/CreateTokenLambda"
COPY . .
RUN dotnet build "CreateTokenLambda.csproj" --configuration Release --output /app/build

FROM build AS publish
RUN dotnet publish "CreateTokenLambda.csproj" \
...
Execute unit tests
```
cd "CreateTokenLambda/test/CreateTokenLambda.Tests"
dotnet test
```

Deploy function to AWS Lambda
```
cd "CreateTokenLambda/src/CreateTokenLambda"
